...
function setupDefaultView() { 
    ...
    //var defaultView = { title: "Free Sharing App!",
    var defaultView = { title: "Food Management System",
    view:  bodyView
    };
...
function onCameraViewClick( event ) {
    var view = { title: "Camera",
        backLabel: (isTablet() ? "Back" : " "),
        view: viewAssembler.cameraView()
    };
    window.viewNavigator.pushView( view );
    event.stopPropagation();
    return false;
}
function onPostViewClick( event ) {
    var view = { title: "Post",
        backLabel: (isTablet() ? "Back" : " "),
        view: viewAssembler.postView()
    };
    window.viewNavigator.pushView( view );
    event.stopPropagation();
    return false;
...
<!-- ALL THE JAVASCRIPT IS BELOW -->
window.fbAsyncInit = function() {
    FB.init({
        appId      : '302744386742543',
        xfbml      : true,
        version    : 'v2.7'
    });
};
